...
> [!IMPORTANT] 
> It should be assumed that any element of this list that is **bolded** with 'N/A' in the default column is required.
...
| SAML_METADATA_LOCAL      | str  | ''      | NOT ENABLED, SEE NOTE BELOW - Local SAML2 metadata configuration |
...
| SAML_ATTR_GROUPS         | str  | ''      | Attribute claim for user groups, similar to other SAML_ATTR, will determine where to look for your designated IdP assigned groups |
| SAML_GROUP_PI            | str  | ''      | Name of IdP group for PIs |
| SAML_GROUP_STAFF         | str  | ''      | Name of IdP group for staff |
| SAML_GROUP_USER          | str  | ''      | Name of IdP group for users |
| **SAML_ENTITY_ID**       | str  | N/A     | SAML2 SSO Identity Provider audience URI, *e.g.* http://your-domain/saml2_auth/acs/ |
...
#### SAML_ATTR_*
The ENV variables for email - `SAML_ATTR_EMAIL`, username - `SAML_ATTR_USERNAME`, given name - `SAML_ATTR_FNAME`, surname - `SAML_ATTR_LNAME`, and groups (if enabled) - `SAML_ATTR_GROUPS` are all strings that are configurable in your IdP. In Entra ID, we have it set up as follows:
...
> [!IMPORTANT]
> We've found through testing that this plugin does NOT handle "namespaces," *i.e.* URLs before the claim name, very well. It's best to keep these names to simple alphanumeric lables that relate to what value they're passing.
